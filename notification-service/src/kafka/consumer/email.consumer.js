...
class EmailConsumer {
     ...
     async handleBookingConfirmed(data) {
          ...
          if (!email) { logger.warn(`Skipping booking-confirmed email — no email on event`, { bookingId }); return; }
     ...
     async handleBookingFailed(data) {
          const { email, bookingId } = data;
          if (!email) { logger.warn(`Skipping booking-failed email — no email on event`, { bookingId }); return; }
     ...
     async handleBookingCancelled(data) {
          const { email, bookingId } = data;
          if (!email) { logger.warn(`Skipping booking-cancelled email — no email on event`, { bookingId }); return; }
