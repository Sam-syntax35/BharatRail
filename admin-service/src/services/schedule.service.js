...
const createSchedule = async (data) => {
     ...
     const eventPayload = {
          ...
          route: train.route.routeStations.map((rs) => ({
               ...
               city: rs.station.city, sequenceNumber: rs.sequenceNumber,
               arrivalTime: rs.arrivalTime, departureTime: rs.departureTime,
               distanceFromOrigin: rs.distanceFromOrigin,
          })),
     };

     // FIXED — .catch() added, matching the fire-and-forget pattern used elsewhere
...
const cancelSchedule = async (scheduleId) => {
     ...
     // FIXED — .catch() added here too
     await adminProducer.publishScheduleCancelled(updated).catch((err) => {
