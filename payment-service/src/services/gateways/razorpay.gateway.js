...
class RazorpayGateway extends BaseGateway {
     ...
     async createOrder(amount, currency, receipt, notes = {}) {
          ...
          let order;
          try {
               order = await this.client.orders.create({ amount: amountInPaise, currency, receipt, notes });
          } catch (err) {
     ...
     verifyPaymentSignature(orderId, paymentId, signature) {
          try {
               ...
               logger.warn(`Signature verification failed due to format or length mismatch: ${err.message}`);
               return false;
          }
     }
     ...
     async fetchPayment(paymentId) {
          ...
          return { status: payment.status, amount: payment.amount / 100, method: payment.method, rawResponse: payment };
     }
    ...
    async initiateRefund(paymentId, amount, notes = {}) {
     ...
     let refund;
     try {
          refund = await this.client.payments.refund(paymentId, { amount: amountInPaise, notes });
     } catch (err) {
          const description = err?.error?.description || err?.message || JSON.stringify(err);
          logger.error(`Razorpay initiateRefund failed: ${description}`, { paymentId });
          const { BadRequestError } = require('../../utils/error');
          throw new BadRequestError(`Refund failed: ${description}`, 'REFUND_GATEWAY_ERROR');
     }

     logger.info(`Razorpay refund initiated: ${refund.id}`, { paymentId, amount });
     return { gatewayRefundId: refund.id, status: refund.status, amount: refund.amount / 100, rawResponse: refund };
}

     async fetchRefund(paymentId, refundId) {
          const refund = await this.client.payments.fetchRefund(paymentId, refundId);
          return { status: refund.status, amount: refund.amount / 100, rawResponse: refund };
     }
}
