...
exports.lockSeats = asyncHandler(async (req, res) => {
     const { scheduleId, seatIds, ttlSeconds, userId, fromSeq, toSeq } = req.body;
     ...
     const result = await inventoryService.lockSeats(scheduleId, seatIds, userId, ttlSeconds, fromSeq, toSeq);

     res.status(200).json({
          success: true,
          message: `${result.lockedSeats.length} seat(s) locked successfully`,
          data: { scheduleId: result.scheduleId, lockedSeats: result.lockedSeats, lockExpiresAt: result.lockExpiresAt },
     });
});
...
exports.unlockSeats = asyncHandler(async (req, res) => {
     ...
     const result = await inventoryService.unlockSeats(scheduleId, seatIds, userId, fromSeq, toSeq);

     res.status(200).json({
          success: true,
          message: `${result.unlockedSeats.length} seat(s) unlocked successfully`,
          data: { scheduleId: result.scheduleId, unlockedSeats: result.unlockedSeats },
     });
});

exports.confirmSeats = asyncHandler(async (req, res) => {
     const { scheduleId, seatIds, bookingId, userId, fromSeq, toSeq } = req.body;
     ...
     const result = await inventoryService.confirmSeats(scheduleId, seatIds, userId, bookingId, fromSeq, toSeq);

     res.status(200).json({
          success: true,
          message: `${result.confirmedSeats.length} seat(s) confirmed`,
          data: { scheduleId: result.scheduleId, bookingId: result.bookingId, confirmedSeats: result.confirmedSeats },
     });
});
...
exports.cancelBooking = asyncHandler(async (req, res) => {
     ...
     if (!scheduleId || !bookingId) throw new BadRequestError('scheduleId and bookingId are required');
     if (!userId) throw new BadRequestError('userId is required');
     ...
     res.status(200).json({
          ...
          data: { scheduleId: result.scheduleId, bookingId: result.bookingId, releasedSeats: result.releasedSeats },
     });
});