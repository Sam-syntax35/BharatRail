...
const indexTrainRoute = async (routeEvent) => {
     ...
     await esClient.index({ index: TRAIN_INDEX, id: train.id, document: doc, refresh: true });
     ...
     for (const rs of routeStations) {
          await esClient.index({
               ...
               document: {
                    ...
                    suggest: { input: [rs.station.name, rs.station.code, rs.station.city].filter(Boolean), weight: 10 },
               },
               refresh: true,
          });
     }
...
const indexSchedule = async (scheduleEvent) => {
     ...
     try {
          ...
          `,
                    params: {
                         scheduleId,
                         newSchedule: { scheduleId, departureDate, status, available: totalSeats, locked: 0, booked: 0 },
                    },
               },
               refresh: true,
          });
          ...
          await esClient.update({
            ...
            if (ctx._source.schedules != null) {
              for (def s : ctx._source.schedules) {
                if (s.scheduleId == params.scheduleId) { s.status = 'CANCELLED'; }
              }
            }
          `,
...
const searchTrains = async (from, to, date) => {
     ...
     const query = {
          bool: {
               must: [
                    { nested: { path: 'route', query: { term: { 'route.stationId': fromStation.stationId } }, inner_hits: { name: 'from_station' } } },
                    { nested: { path: 'route', query: { term: { 'route.stationId': toStation.stationId } }, inner_hits: { name: 'to_station' } } },
               ],
          },
     };
     ...
     const trains = result.hits.hits
          .map((hit) => {
               ...
               if (!fromHit || !toHit || fromHit.sequenceNumber >= toHit.sequenceNumber) return null;

               let scheduleInfo = null;
               if (date && src.schedules && src.schedules.length > 0) {
                    scheduleInfo = src.schedules.find((s) => s.status === 'ACTIVE' && normalize(s.departureDate) === date) || null;
               }
...
const resolveStation = async (input) => {
     const exactResult = await esClient.search({ index: STATION_INDEX, query: { term: { code: input.toUpperCase() } }, size: 1 });
     if (exactResult.hits.hits.length > 0) return exactResult.hits.hits[0]._source;

     try {
          const suggestResult = await esClient.search({
               index: STATION_INDEX,
               suggest: { station_suggest: { prefix: input, completion: { field: 'suggest', fuzzy: { fuzziness: 'AUTO' }, size: 1 } } },
          });
     ...
     const fuzzyResult = await esClient.search({
          index: STATION_INDEX,
          query: { multi_match: { query: input, fields: ['name', 'city'], fuzziness: 'AUTO', prefix_length: 1 } },
          size: 1,
     });
...
const autocompleteStation = async (prefix) => {
     const result = await esClient.search({
          index: STATION_INDEX,
          suggest: { station_suggest: { prefix, completion: { field: 'suggest', fuzzy: { fuzziness: 'AUTO' }, size: 10 } } },
     });
...
const getAllStations = async () => {
     const result = await esClient.search({ index: STATION_INDEX, query: { match_all: {} }, size: 100 });
     return result.hits.hits.map((h) => h._source);
};

const getAllTrains = async () => {
     const result = await esClient.search({ index: TRAIN_INDEX, query: { match_all: {} }, size: 100 });
     return result.hits.hits.map((h) => h._source);
};

module.exports = {
     indexStation, indexTrainRoute, indexSchedule, cancelSchedule, updateSeatAvailability,
     searchTrains, autocompleteStation, getAllStations, getAllTrains,
};